...
class UrerRegistration {
 ...
 public:
  ...
  std::string name, second_name, gender;
  int age;
  struct Personal {
    std::string name;
  ...
  void Update();
  void getPersonalInfo(string& n_name, string& n_second_name, string& n_gender,
                       int& n_age, vector<cv::Mat>& n_photos);
};
...
void UrerRegistration::CreateRegistrationSuccesCanvas() {
  ...
  main_canvas.ClearCanvas();
  storage.push_back(Personal(name, second_name, gender, age, images));
  ...
  main_canvas.AddUIElement(new Button(
      cv::Point(1280 - 200 - 100, 720 / 2 - 25 + 75), cv::Size(200, 50),
      [&]() { main_canvas.Exit(); }, CV_RGB(0, 0, 0), "   Exit   "));
...
void UrerRegistration::CreateRegistrationFailCanvas(string error) {
  ...
  main_canvas.AddUIElement(new Button(
      cv::Point(1280 - 200 - 300, 720 / 2 - 25 + 75), cv::Size(200, 50),
      [&]() { main_canvas.Exit(); }, CV_RGB(0, 0, 0), "   Exit   "));
}
void UrerRegistration::CreateInputCanvas(std::vector<string> titles = {}, std::vector<string> defaults = {}) {
  ...
  for (int i = 0; i < titles.size(); ++i) {
    std::shared_ptr<InputField> tmp_input_field(new InputField(i == 0, titles[i], defaults[i], cv::Point(50, 100 + i * 80)));
    input_fields.push_back(tmp_input_field);
    main_canvas.AddUIElement(tmp_input_field.get());
  ...
  if (all_active && start_input) {
    ...
    main_canvas.AddUIElement(new Button(
        ...
        [&]() {
          ...
          try {
            age = std::stoi(input_fields[3]->getText());
...
void UrerRegistration::getPersonalInfo(string& n_name, string& n_second_name,
    string& n_gender, int& n_age,
    vector<cv::Mat>& n_photos) {
  n_name = name;
  n_second_name = second_name;
  n_gender = gender;
  n_age = age;
  n_photos = images;
}
